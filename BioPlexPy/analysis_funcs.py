...
def permutation_test_for_CORUM_complex(bp_PPI_G, Corum_DF, Complex_ID, num_perms = 1000):
    ...
    proteins (with the same number of baits as the CORUM complex) and storing this value (E_i), 
    ...
    while S_i < num_perms:
        ...
        # check to see if nodes in subgraph have the same number of baits as the subgraph induced by the CORUM complex
        bp_S_baits_num = np.sum([bp_PPI_S.nodes[node_i]['bait'] for node_i in bp_PPI_S.nodes])

        # number of baits in CORUM complex & S are the same
        if bp_S_baits_num == bp_complex_i_baits_num:
